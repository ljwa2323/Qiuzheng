...
浏览器原型阶段使用 `localStorage` 演示。生产化代码位于 `apps/api` 与 `apps/web`：身份鉴权、项目权限隔离、PostgreSQL 持久化、MinIO 文件存储、异步任务与用户自备 Key 的多厂商模型网关已落地。全文 PDF 真解析、完整 RoB/提取/裁决深化与 SSO 仍属后续阶段。
